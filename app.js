...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initMap() {
        ...
        // Base layer: CARTO Voyager — detailed streets, labels, terrain
        ...
        // Aeronautical overlay: OpenAIP airport/airspace data
        L.tileLayer('https://api.tiles.openaip.net/api/data/openaip/{z}/{x}/{y}.png?apiKey=public', {
    ...
    // Primary: adsb.lol — public ADS-B aggregator, no auth, no CORS issues
    // Fallback: OpenSky via CORS proxies
    // Static fallback: flights.json generated by GitHub Actions
    ...
    // Parse adsb.lol aircraft array into our internal flight format
    function parseAdsbLol(aircraftList) {
        const VABB = [19.0896, 72.8656];
        const VANM = [18.9919, 73.0617];
        const flights = [];

        aircraftList.forEach(a => {
            const icao = a.hex;
            if (!icao) return;
            const callsign = (a.flight || a.r || icao).trim();
            const lat = a.lat;
            const lon = a.lon;
            if (lat === undefined || lon === undefined) return;

            const alt  = a.alt_baro !== undefined ? (a.alt_baro === 'ground' ? 0 : Math.round(a.alt_baro)) : 0;
            const speed = a.gs !== undefined ? Math.round(a.gs) : 0;
            const heading = a.track !== undefined ? Math.round(a.track) : 0;
            const vrate = a.baro_rate !== undefined ? Math.round(a.baro_rate) : 0;
            const onGround = (a.alt_baro === 'ground') || alt < 50;
            const squawk = a.squawk || '----';

            const distVabb = calculateDistance(lat, lon, VABB[0], VABB[1]);
            const distVanm = calculateDistance(lat, lon, VANM[0], VANM[1]);
            const closerAirport = distVabb < distVanm ? 'VABB' : 'VANM';
            const closerDist = Math.min(distVabb, distVanm);

            const airlineInfo = getAirlineInfo(callsign);

            let flightType = 'arrival', status = 'En Route';
            if (onGround) {
                flightType = 'on_ground';
                status = closerDist < 2 ? 'Landed' : 'Taxiing';
            } else if (vrate < -200) {
                flightType = 'arrival';
                status = closerDist < 15 ? 'Final Approach' : 'Descending';
            } else if (vrate > 200) {
            ...
            let acModel = a.t || 'B738';
            if (!acModel || acModel.length < 2) {
                if (callsign.startsWith('IGO')) acModel = 'A20N';
                else if (callsign.startsWith('AIC')) acModel = 'B788';
                else if (callsign.startsWith('VTI')) acModel = 'A321';
                else if (callsign.startsWith('UAE')) acModel = 'B77W';
                else if (callsign.startsWith('SIA')) acModel = 'A359';
                else acModel = 'B738';
            }
            ...
            flights.push({
                ...
                origin: meta.origin_city,
                destination: meta.destination_city,
                lat, lon, altitude: alt, speed, heading,
                vertical_rate: vrate,
                squawk,
                registration: a.r || meta.registration,
                status, gate: onGround ? (a.squawk || 'APRON') : '',
                type: flightType,
                airport: closerAirport,
                trail: [[lon, lat]],
                lastUpdatedTime: Date.now(),
                baseLat: lat, baseLon: lon,
                baseAltitude: alt, baseSpeed: speed, baseHeading: heading,
                ...meta
            });
        });
        return flights;
    ...
    // Load Data — adsb.lol primary → OpenSky CORS fallback → flights.json static
    async function loadData(isInterval = false) {
        let loadedFromApi = false;
        let source = '';

        // ── 1. Try adsb.lol (no CORS, no auth needed) ──
        const bbox = {
            lat_min: 18.0, lon_min: 71.5,
            lat_max: 20.5, lon_max: 74.5
        };
        const adsbUrl = `https://api.adsb.lol/v2/lat/${(bbox.lat_min+bbox.lat_max)/2}/lon/${(bbox.lon_min+bbox.lon_max)/2}/dist/250`;

        try {
            const res = await fetch(adsbUrl, { signal: AbortSignal.timeout(7000) });
            if (!res.ok) throw new Error(`adsb.lol HTTP ${res.status}`);
            const data = await res.json();
            const aircraft = data.ac || data.aircraft || [];
            // Filter to our bounding box
            const inBbox = aircraft.filter(a =>
                a.lat >= bbox.lat_min && a.lat <= bbox.lat_max &&
                a.lon >= bbox.lon_min && a.lon <= bbox.lon_max
            );
            if (inBbox.length > 0) {
                state.flights = parseAdsbLol(inBbox);
                loadedFromApi = true;
                source = `adsb.lol • ${state.flights.length} targets`;
                console.log(`✅ ${state.flights.length} live targets from adsb.lol`);
            }
        } catch (err) {
            console.warn(`⚠️ adsb.lol failed: ${err.message}`);
        }

        // ── 2. OpenSky via CORS proxy ──
        if (!loadedFromApi) {
            const STALE_THRESHOLD = 300;
            const now = Math.floor(Date.now() / 1000);
            const openskyUrl = `https://opensky-network.org/api/states/all?lamin=${bbox.lat_min}&lomin=${bbox.lon_min}&lamax=${bbox.lat_max}&lomax=${bbox.lon_max}`;
            ...
            for (const buildProxy of CORS_PROXIES) {
                ...
                try {
                    ...
                    if (data && data.states && data.states.length > 0) {
                        const freshStates = data.states.filter(s => s[4] && (now - s[4]) <= STALE_THRESHOLD);
                        if (freshStates.length > 0) {
                            state.flights = parseOpenSkyData({ ...data, states: freshStates });
                            loadedFromApi = true;
                            source = `OpenSky • ${state.flights.length} targets`;
                            console.log(`✅ ${state.flights.length} targets from OpenSky proxy`);
                        }
                    }
                } catch (err) {
                    console.warn(`⚠️ OpenSky proxy failed: ${err.message}`);
                }
            }
        }
        ...
        if (!loadedFromApi) {
            try {
                ...
                if (!res.ok) throw new Error('flights.json unavailable');
                state.flights = await res.json();
                state.flights.forEach(f => {
                    f.lastUpdatedTime = Date.now();
                    f.baseLat = f.lat; f.baseLon = f.lon;
                    f.baseAltitude = f.altitude; f.baseSpeed = f.speed; f.baseHeading = f.heading;
                });
                source = `static cache • ${state.flights.length} targets`;
                console.log(`💾 Loaded from flights.json`);
            } catch (err) {
    ...
    function updateTimestamp(source = '') {
        ...
        elements.lastUpdatedText.textContent = `Updated: ${timeString}${srcLabel}`;
    }
